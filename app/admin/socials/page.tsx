...
import Skeleton from "./components/ocialsSkeleton"; // можно сделать отдельный Skeleton для соцсетей
...
export default function AdminSocials() {
  ...
  const [formData, setFormData] = useState<CreateSocialNetworkPayload>({
    name: "",
    url: ""
  });
  ...
  const totalPages = Math.ceil(filteredSocials.length / itemsPerPage);
    ...
    <div className="space-y-8 p-4 sm:p-6 lg:p-8 bg-gray-50 w-full">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 flex-1">
          <StatCard title="Всего соцсетей" value={socials.length.toString()} icon="🌐" />
        </div>
        ...
        <table className="w-full text-left table-auto">
          <thead>
            <tr className="bg-gray-100">
              ...
              <th className="px-4 py-2 text-sm font-medium text-gray-600">Ссылка</th>
              <th className="px-4 py-2 text-sm font-medium text-gray-600">Дата создания</th>
          ...
          {isLoading ? <Skeleton /> : (
            <tbody>
              {paginatedSocials.map(s => (
                <tr key={s.id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <a href={s.url} target="_blank" className="text-blue-500 underline">{s.url}</a>
                  </td>
              ...
              {paginatedSocials.length === 0 && !isLoading && (
                <tr>
                  <td colSpan={4} className="text-center py-4">Соцсети не найдены</td>
                </tr>
              )}
            </tbody>
          )}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 w-full max-w-md sm:max-w-lg max-h-[90vh] overflow-y-auto shadow-lg">
            <h2 className="text-lg font-semibold mb-4">
              {editingSocial ? "Редактировать соцсеть" : "Добавить соцсеть"}
            </h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button type="submit" className="px-4 py-2 bg-orange-500 text-white rounded-md">{editingSocial ? "Сохранить" : "Добавить"}</button>
              </div>
            </form>
          </div>
        </div>
